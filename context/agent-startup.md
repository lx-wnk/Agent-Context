...
`.agent-context/scripts/agent-context-update.sh` at every session start. The script checks for new releases, updates
shared files (including the script itself), and syncs plugins — no agent involvement needed.

For non-Claude-Code agents: check `.agent-context/.agent-context-version` against
