...
- **Generated artifacts** (`outputs/`): Reports produced by skills — review findings, handoffs, audits, analyses. Untracked, never installed, never referenced by shared files or tests. Regenerate rather than edit; deleting the directory must never break anything. Not in `.gitignore` yet, so keep it out of `git add -A`.
